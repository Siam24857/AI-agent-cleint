...
export default function ResumeAnalyzePage() {
  ...
  const ScoreCircle = ({ value, label, color }: { value: number; label: string; color: string }) => (
    <div className="flex flex-col items-center">
      ...
      >
        <div className="w-16 h-16 rounded-full bg-white flex items-center justify-center text-gray-900">
          {value}
        </div>
      </div>
        ...
        {!result && (
          <div className="bg-white rounded-xl shadow p-8">
            ...
            <button
              ...
              className="mt-6 w-full bg-indigo-600 text-white py-3 rounded-md font-semibold hover:bg-indigo-700 disabled:opacity-50 flex items-center justify-center gap-2"
            >
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-xl shadow p-8">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                <ScoreCircle value={analysis.score} label="Overall" color="#4f46e5" />
                <ScoreCircle value={analysis.atsScore} label="ATS Score" color="#0ea5e9" />
                <ScoreCircle value={analysis.keywordScore} label="Keywords" color="#10b981" />
                <ScoreCircle value={analysis.readabilityScore} label="Readability" color="#f59e0b" />
              </div>
              {analysis.summary && (
                <p className="mt-6 text-gray-700 bg-gray-50 rounded-lg p-4">{analysis.summary}</p>
              )}
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="bg-white rounded-xl shadow p-6">
                <h3 className="flex items-center gap-2 font-semibold text-gray-900 mb-3">
                  <AlertTriangle className="h-5 w-5 text-red-500" /> Weaknesses
                </h3>
                <ul className="space-y-2">
            ...
            <div className="bg-white rounded-xl shadow p-6">
              <h3 className="flex items-center gap-2 font-semibold text-gray-900 mb-3">
                <Gauge className="h-5 w-5 text-indigo-600" /> Missing Skills
              </h3>
            ...
            <div className="bg-white rounded-xl shadow p-6">
              ...
              <ul className="space-y-2">
                {analysis.suggestions?.map((s: string, i: number) => (
                  <li key={i} className="flex gap-2 text-sm text-gray-700">
                    <Lightbulb className="h-4 w-4 text-yellow-500 mt-0.5 shrink-0" /> {s}
                  </li>
                ))}
              </ul>
            </div>
